...
export async function getTuteeDashboard(userId: string) {
  let upcoming: any[] = [];
  let past: any[] = [];
  try {
    upcoming = await query<any[]>(
      `SELECT s.id AS id, p.first_name AS tutor_first_name, p.last_name AS tutor_last_name,
    ...
    past = await query<any[]>(
      `SELECT s.id, s.session_date, s.start_time, s.status, s.payment_status, c.name AS course_name,
    ...
    past = await query<any[]>(
      `SELECT s.id, s.session_date, s.start_time, s.status, c.name AS course_name,
              p.first_name AS tutor_first_name, p.last_name AS tutor_last_name,
              r.id AS review_id
       FROM sessions s
       JOIN tutor_profiles tp ON s.tutor_id = tp.id
...
export async function getTutorDashboard(userId: string, earningsRange?: string) {
  ...
  const range =
    earningsRange === "day" || earningsRange === "week" || earningsRange === "month"
      ? earningsRange
      : "month";
  const rangeFilter =
    range === "day"
  ...
  try {
    upcoming = await query<any[]>(
      `SELECT s.id, s.session_date, s.start_time, s.status, s.payment_status, c.name AS course_name,
       ...
       ORDER BY s.session_date ASC, s.start_time ASC
       LIMIT 10`,
      { tutor_id: tutorId, tutor_user_id: userId }
    );
    past = await query<any[]>(
      `SELECT s.id, s.session_date, s.start_time, s.status, s.payment_status, c.name AS course_name,
              p.first_name AS tutee_first_name, p.last_name AS tutee_last_name
  ...
  let requests: any[] = [];
  try {
    requests = await query<any[]>(
      `SELECT sr.id, sr.created_at, c.name AS course_name,
              p.first_name AS tutee_first_name, p.last_name AS tutee_last_name,
