...
type ReportRole = "admin" | "tutor" | "tutee";

const DATE_PATTERN = /^\d{4}-\d{2}-\d{2}$/;

const parseRange = (value: unknown): RangeValue => {
...
const resolveRange = (rangeParam: unknown, startParam: unknown, endParam: unknown) => {
...
const getDefaultRole = (roles: string[]): ReportRole => {
...
router.get("/tutee", requireAuth, requireRole(["tutee", "admin"]), async (req, res, next) => {
  try {
    ...
    const rows = await query<any[]>(
      `SELECT d.day, COALESCE(s.count, 0) AS count
       FROM (
       ...
       ORDER BY d.day ASC`,
      { user_id: req.user!.id, days_back: daysBack }
    );
    res.json({ data: rows });
  } catch (err) {
    next(err as Error);
  ...
  try {
    ...
    const rows = await query<any[]>(
      `SELECT d.day, COALESCE(s.count, 0) AS count
       ...
       WHERE d.day >= DATE_SUB(CURDATE(), INTERVAL :days_back DAY)
       ORDER BY d.day ASC`,
      { tutor_id: tutorId, days_back: daysBack }
    );
    res.json({ data: rows });
  } catch (err) {
    next(err as Error);
...
router.get("/report", requireAuth, async (req, res, next) => {
  try {
    ...
    const allowedRoles = ["admin", "tutor", "tutee"] as const;

    if (requestedRole && !allowedRoles.includes(requestedRole as ReportRole)) {
    ...
    const role = (requestedRole ?? defaultRole) as ReportRole;
    ...
    if (role === "tutor") {
      const tutorRows = await query<{ id: string }[]>(
        "SELECT id FROM tutor_profiles WHERE user_id = :user_id",
        { user_id: req.user!.id }
      );
    ...
    const seriesIndex = new Map(
      seriesRows.map((row) => [row.day, { count: Number(row.count), revenue: Number(row.revenue) }])
    );
    ...
    const paymentRows = await query<any[]>(
      `SELECT s.payment_status AS status, COUNT(*) AS count
       FROM sessions s
