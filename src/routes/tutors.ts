...
router.get("/", validate(searchSchema), async (req, res, next) => {
  try {
    const { search, courseId, departmentId, minRating, featured, limit, offset } =
...
const updateProfileSchema = z.object({
  body: z.object({
    ...
    phone: z
      ...
      .max(20)
      .optional()
      .refine((value) => !value || isValidGhanaPhone(value), {
...
router.get("/me", requireAuth, requireVerified, requireRole(["tutor"]), async (req, res, next) => {
  try {
    const rows = await query<any[]>(
      `SELECT 
          tp.id AS tutor_id,
    ...
    if (!tutor) {
      await query(
        "INSERT INTO tutor_profiles (user_id, headline, is_verified, is_available) VALUES (:user_id, :headline, FALSE, FALSE)",
        { user_id: req.user!.id, headline: "New Tutor" }
      );
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!tutorId) {
        return next(notFound("Tutor profile not found"));
      }
      await query(
      ...
      if (typeof req.body.phone === "string") {
        await query(
          "UPDATE profiles SET phone = :phone WHERE user_id = :user_id",
          { phone: req.body.phone, user_id: req.user!.id }
        );
      }
      res.json({ ok: true });
    } catch (err) {
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      await query("DELETE FROM tutor_courses WHERE tutor_id = :tutor_id", {
        tutor_id: tutorId,
      });
      const courseIds: string[] = req.body.courseIds ?? [];
      for (const courseId of courseIds) {
        await query(
          "INSERT INTO tutor_courses (id, tutor_id, course_id) VALUES (UUID(), :tutor_id, :course_id)",
          { tutor_id: tutorId, course_id: courseId }
        );
      }
      res.json({ ok: true });
    } catch (err) {
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    if (!tutor) {
      return next(notFound("Tutor not found"));
    }
    const courses = await query<any[]>(
    ...
    const reviews = await query<any[]>(
      `SELECT r.id, r.overall_rating, r.review_text, r.created_at,
...
router.get("/:id/availability", async (req, res, next) => {
  try {
    ...
    if (!date) {
      return next(badRequest("date is required"));
    }
